...
public class MeiTuanStoreDataServiceImpl extends AbstractHandlerStore {
    ...
    public void getStoreData(String keyword) {
        String jsonResult = wrapParameters(keyword, 10, 0);
        log.info(jsonResult);
    ...
    private void uploadData(MeiTuanData.DataEntity.SearchResultEntity searchResultEntity, String kerword) {
        ...
        try {
            ...
            log.info("写入success,id: {} ,名称 : {} ,手机号 : {}", searchResultEntity.getId(), searchResultEntity.getTitle(), searchResultEntity.getPhone());
        } catch (DuplicateKeyException e) {
            log.error("商店id已存在,id: {} ,名称 : {}", searchResultEntity.getId(), searchResultEntity.getTitle());
        } catch (Exception e) {
            log.error("写入异常.", e);
        }
    }
     ...
     * @return
     */
    public String wrapParameters(String keyword, int limit, int offset) {
